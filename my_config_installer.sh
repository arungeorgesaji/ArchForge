...
sudo pacman -S wget neofetch openssh git nano vim neovim rofi kitty ly godot blender gimp inkscape krita obs-studio lf xorg-server xorg-xinit xorg-apps plasma-desktop pavucontrol blueman shotcut uget qbittorrent feh dunst pamixer pulseaudio upower scrot xclip ardour tldr cups python lf kitty libvirt virt-manager qemu-base vlc unzip tar vivaldi firefox steam 
...
yay -S linux-wifi-hotspot yaycache autojump picom-git python39 python310 trash-cli wireshark-git ms-365-electron-bin conda fastfetch-git postman-bin ani-cli debtap
